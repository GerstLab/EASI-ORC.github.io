...
for (img = 0; img < cell_list.length; img++) {
	...
	if (File.isFile(cell_img_path)) {
		...
		if (img_width > 1024 || img_height > 1024) {
			run("Size...", "width=1024 height=1024 depth=12 constrain interpolation=Bicubic");
		}
		...
		run("Size...", "width=" + img_width + " height=" + img_height + " depth=1 constrain interpolation=Bicubic");
		run("Convert to Mask");
